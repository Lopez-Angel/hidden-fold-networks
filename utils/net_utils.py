# from https://github.com/allenai/hidden-networks
...
def freeze_model_weights(model):
    ...
    for n, m in model.named_modules():
        if not hasattr(m, "affine") or m.affine is not True: #Protects UBN
...
def set_model_top_k(model, top_k):
    print(f"==> Setting prune rate of network to {top_k}")

    for n, m in model.named_modules():
        if hasattr(m, "set_top_k"):
            m.set_top_k(top_k)
            print(f"==> Setting prune rate of {n} to {top_k}")
